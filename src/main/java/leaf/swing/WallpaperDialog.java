...
public final class WallpaperDialog extends LeafDialog {
	...
	public WallpaperDialog(Window owner) {
		...
		chooser.addChoosableFileFilter(new FileNameExtensionFilter("JPEG / PNG / GIF", "jpg", "jpeg", "png", "gif"));
	}
	...
	public void initialize() {
		...
		add(button_cancel);
		var ed = (NumberEditor) spin_trans.getEditor();
		ed.getTextField().setEditable(false);
		layoutComponents();
	}

	private void layoutComponents() {
		var pref_h = button_open.getPreferredSize().height;
		var b = panel_photo.getBorder().getBorderInsets(panel_photo);
	...
	public boolean showDialog(WallpaperSettings ws) {
		...
		var trans = 100 - (int) (ws.getPhotoAlpha() * 100);
		...
		if (!check_enable.isSelected()) settings.setPhotoFilePath(null);
		return OK_OPTION;
	}
	...
	private class OpenFileAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			var ret = chooser.showOpenDialog(getContentPane());
	...
	private class SelectColorAction extends AbstractAction {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			var color = JColorChooser.showDialog(getContentPane(), getTitle(), settings.getBackgroundColor());
			if (color != null) {
